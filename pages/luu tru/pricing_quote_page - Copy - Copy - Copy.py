...
def _preview_internal_cost(
    ...
    Logic cập nhật:
    🔸 Giữ nguyên toàn bộ quy trình tính toán gốc
    🔸 Chỉ thay đổi phần lọc ngày hiệu lực: so sánh theo ngày-tháng (MMDD),
       không gán năm cho EffectiveDate / ExpirationDate.
    """
    ...
    # ---- Kiểm tra đủ rates cho container plan ----
    ...
    # (So sánh chỉ theo ngày-tháng, không ép năm)
    # ====================================================
    today = pd.Timestamp.today()
    today_md = int(today.strftime("%m%d"))

    def to_md(d_str):
        d = pd.to_datetime(str(d_str), format="%d-%b", errors="coerce")
        if pd.isna(d):
            return None
        return int(d.strftime("%m%d"))

    df_valid["Eff_md"] = df_valid["EffectiveDate"].apply(to_md)
    df_valid["Exp_md"] = df_valid["ExpirationDate"].apply(to_md)

    df_valid = df_valid[
        (df_valid["Eff_md"].notna()) &
        (df_valid["Exp_md"].notna()) &
        (df_valid["Eff_md"] <= today_md) &
        (df_valid["Exp_md"] >= today_md)
    ].copy()

    if df_valid.empty:
        return {
            "error": "NO_VALID_RATE_FOR_TODAY",
            "message": f"Không có dòng giá nào còn hiệu lực tại ngày hôm nay ({today.strftime('%d-%b')})."
        }
