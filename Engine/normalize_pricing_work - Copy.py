...
def add_delta_display_columns(master_current, master_previous):
    ...
    # Join current vs previous by business keys (route/carrier/contract/commodity/rate/...)
    # Ignore EffectiveDate / ExpirationDate so that we can compare different periods
    # of the same lane.
    ...
    def format_delta_icon(delta):
        ...
        if np.isclose(delta_val, 0):
            ...
            icon = "▼"
        if float(delta_val).is_integer():
            delta_str = str(int(delta_val))
        else:
            delta_str = str(round(delta_val, 2))
        return icon + " " + delta_str
    ...
    for cont in container_cols:
        ...
        display_col = "DELTA_" + cont + "_DISPLAY"
        df[delta_col] = df[price_col] - df[old_col]
        df[display_col] = df[delta_col].apply(format_delta_icon)

        delta_display_cols.append(display_col)
    ...
    extra_cols = [
        ...
        for c in df.columns
        if c
        not in base_cols + price_cols + old_cols_to_drop + numeric_delta_cols + delta_display_cols
    ]

    ordered_cols = base_cols + price_cols + delta_display_cols + extra_cols
...
def apply_delta_icon_format(ws, col_letter, start_row, end_row):
    if end_row < start_row:
        return

    cell_range = "{0}{1}:{0}{2}".format(col_letter, start_row, end_row)
    ...
    ws.conditional_formatting.add(
        cell_range,
        FormulaRule(
            formula=['=LEFT({0}{1},1)="▲"'.format(col_letter, start_row)],
            font=green_font,
        ),
    )
    ws.conditional_formatting.add(
...
def combine_all(list_df, include_expired: bool = False, cutoff_date: date | None = None):
    ...
    with pd.ExcelWriter(master_file, engine="openpyxl") as writer:
        ...
        for header in display_headers:
            ...
            if col_letter:
                apply_delta_icon_format(ws, col_letter, data_start_row, data_end_row)
