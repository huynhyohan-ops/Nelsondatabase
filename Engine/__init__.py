...
# === AUTO-RESOLVED PROJECT ROOT ===
# /PricingSystem/App/quote_engine_gui.py → BASE_DIR = /PricingSystem
...
MASTER_FILE = os.path.join(BASE_DIR, "Master_FullPricing.xlsx")
QUOTE_COUNTER_FILE = os.path.join(BASE_DIR, "quote_counters.csv")

# thư mục lưu log nội bộ
LOG_DIR = os.path.join(BASE_DIR, "Quotes_Log")
...
def load_master(master_path: str = MASTER_FILE) -> pd.DataFrame:
    """
...
def generate_quote(master_df: pd.DataFrame, req: QuoteRequest) -> Dict[str, Any]:
    ...
    # ---- SUMMARY (bổ sung email, contact_person, sales_person) ----
    summary = {
...
def build_quote_ref(req: QuoteRequest) -> str:
    ...
    df_ct.to_csv(QUOTE_COUNTER_FILE, index=False)
