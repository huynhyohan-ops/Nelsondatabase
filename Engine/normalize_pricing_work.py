...
def normalize_pod_column(df, pod_map):
    df = df.copy()
    df["POD"] = df["POD"].apply(
        lambda x: pod_map.get(str(x).strip().upper(), str(x).strip()) if pd.notna(x) else x
    )
    return df
...
def combine_all(list_df):
    ...
    # CHUẨN HÓA PlaceOfDelivery (viết hoa, bỏ space)
...
def normalize_all_from_streamlit(raw_dir_override=None, data_dir_override=None):
    """
    Wrapper để chạy toàn bộ pipeline normalize (như main()) ngay trong môi trường Streamlit.

    - Giữ nguyên 100% logic cũ (normalize_file + combine_all).
    - Cho phép override thư mục Raw/Data để đồng bộ với cấu trúc project của Streamlit.
    """
    global raw_dir, data_dir, master_file, puc_file
    ...
    try:
        ...
        # Trả về đường dẫn master_file mới tạo
        return master_file

    finally:
        # Khôi phục cấu hình global ban đầu để không ảnh hưởng khi chạy CLI
        raw_dir = old_raw_dir
