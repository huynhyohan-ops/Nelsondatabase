...
def get_local_charges_pol() -> List[dict]:
    """Chuẩn hóa và bổ sung cột 40RF & 45."""
    """Local charges @ POL theo layout anh chốt."""
    # Dữ liệu 40RF & 45 được đưa vào cột riêng (amt40rf) cho từng dòng
    # Không còn dùng dòng riêng "40RF & 45 (USD)" nữa
    return [
        {"name": "THC", "basis": "Per container", "amt20": 135, "amt40": 200, "amt40rf": 280},
        {"name": "SEAL", "basis": "Per piece", "amt20": 10, "amt40": 10, "amt40rf": 10},
        {"name": "AMS", "basis": "Per shipment", "amt20": 40, "amt40": 40, "amt40rf": 40},
        {"name": "Telex Release", "basis": "Per shipment", "amt20": 35, "amt40": 35, "amt40rf": 35},
        {"name": "House Bill", "basis": "Per set", "amt20": 40, "amt40": 40, "amt40rf": 40},
    ]
...
def generate_quote_pdf(result: Dict[str, Any], logo_path: str) -> str:
    ...
    # Via Port: ưu tiên via_port từ summary (đã lấy từ option recommend / POD Master Pricing)
    via_port = (
        summary.get("via_port")     # từ engine (POD dòng giá recommend)
        or summary.get("pod")       # filter user chọn (fallback)
        or summary.get("pod_name")
        or summary.get("pod_display")
        or ""
    )
    ...
    ship_data = [
        ...
        [Paragraph("<b>Via Port:</b>", style_normal), Paragraph(via_port, style_normal)],
    ...
    # Determine container columns (dynamic)
    preferred_order = ["20GP", "40GP", "40HQ", "45HQ", "20RF", "40RF", "40NOR"]

    containers = result.get("containers")
    if not containers:
        found = set()
        for opt in options:
            for ct in opt.get("container_rates", {}).keys():
                found.add(ct)
        containers = [c for c in preferred_order if c in found] + [
            c for c in sorted(found) if c not in preferred_order
        ]

    ocean_header = ["Carrier", "Validity", "Transit Time"] + containers
    ...
    for opt in options:
        ...
        row = [
            opt.get("carrier", ""),
            validity,
            opt.get("transit_time", ""),
        ]
        for ct in containers:
            row.append(rates.get(ct, ""))
        ocean_data.append(row)

    # Điều chỉnh lại width 3 cột text:
    # Carrier: ngắn hơn
    # Validity: dài hơn
    # Transit Time: gọn lại
    total_width = 170 * mm
    carrier_w = 25 * mm
    validity_w = 40 * mm
    transit_w = 25 * mm
    base_widths = [carrier_w, validity_w, transit_w]

    if containers:
        remaining_width = total_width - sum(base_widths)
        per_col = max(18 * mm, remaining_width / len(containers))
        col_widths = base_widths + [per_col] * len(containers)
    else:
    ...
    ocean_table = Table(
        ...
        hAlign="LEFT",  # bảng căn sát lề trái
    )
    ...
    lc_header = ["Charge Name", "Basis", "20' (USD)", "40' (USD)", "40RF & 45 (USD)"]
    lc_data = [lc_header]

    for lc in local_charges:
        # Đảm bảo không có dòng riêng "40RF & 45 (USD)" nếu local_charges lấy từ nơi khác
        if str(lc.get("name", "")).strip().upper() == "40RF & 45 (USD)".upper():
            continue
        lc_data.append([lc["name"], lc["basis"], lc["amt20"], lc["amt40"], lc.get("amt40rf", "")])

    lc_table = Table(
        lc_data,
        colWidths=[60 * mm, 50 * mm, 20 * mm, 20 * mm, 30 * mm],
    )
    lc_table.setStyle(
        TableStyle(
            [
    ...
    recommended_opt = None    # option recommend (vẽ notes)
    for opt in options:
...
def generate_quote_pdf_multi(results: List[Dict[str, Any]], logo_path: str) -> str:
    ...
    for res in results:
        ...
        ship_data = [
            [Paragraph("<b>Route:</b>", style_normal), Paragraph(route_str, style_normal)],
            [Paragraph("<b>Via Port:</b>", style_normal), Paragraph(via_port_i, style_normal)],
