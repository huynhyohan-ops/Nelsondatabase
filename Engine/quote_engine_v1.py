...
from datetime import date, datetime   # <-- UPDATED: thêm datetime
...
def filter_by_validity(df: pd.DataFrame, cargo_ready_iso: Optional[str]) -> pd.DataFrame:
    """
    Lọc theo ngày hết hạn:
    - Chỉ giữ các dòng có ExpirationDate >= cargo_date
    - Nếu ExpirationDate trống/NaN -> giữ lại (coi như không hết hạn)
    - cargo_ready_iso: chuỗi ISO (YYYY-MM-DD) từ ShipmentInfo.cargo_ready_date
      Nếu None hoặc parse lỗi -> dùng today().
    """
    if "ExpirationDate" not in df.columns:
        return df

    # 1) Xác định ngày mốc (cargo_date): nếu user không nhập thì dùng ngày hiện tại
    if cargo_ready_iso:
        try:
            cargo_date = date.fromisoformat(cargo_ready_iso)
        except Exception:
            cargo_date = date.today()
    else:
        cargo_date = date.today()

    year = cargo_date.year
    col = df["ExpirationDate"]

    # 2) Nếu cột đã là datetime -> dùng trực tiếp
    if pd.api.types.is_datetime64_any_dtype(col):
        exp_date = col.dt.date
    else:
        # Chuyển sang string, chuẩn hoá để parse
        exp_raw = col.astype(str).str.strip().str.upper()

        # Chuyển các giá trị rỗng / "NAN" / "NONE" về NaN
        exp_clean = exp_raw.replace({"": None, "NAN": None, "NONE": None})

        # Pattern kiểu "30NOV" (không có năm)
        mask_short1 = exp_clean.str.match(r"^\d{1,2}[A-Z]{3}$", na=False)
        # Pattern kiểu "30-NOV" (không có năm)
        mask_short2 = exp_clean.str.match(r"^\d{1,2}-[A-Z]{3}$", na=False)

        exp_with_year = exp_clean.copy()

        # "30NOV" -> "30NOV-2024"
        exp_with_year.loc[mask_short1] = (
            exp_clean.loc[mask_short1] + f"-{year}"
        )
        # "30-NOV" -> "30-NOV-2024"
        exp_with_year.loc[mask_short2] = (
            exp_clean.loc[mask_short2] + f"-{year}"
        )

        # Parse linh hoạt (dayfirst=True để tránh nhầm lẫn)
        exp_dt = pd.to_datetime(
            exp_with_year,
            errors="coerce",
            dayfirst=True,
        )
        exp_date = exp_dt.dt.date

    # 3) Rule giữ:
    # - Không parse được ngày (NaT) => giữ (coi như không hạn)
    # - exp_date >= cargo_date => giữ
    mask_keep = exp_date.isna() | (exp_date >= cargo_date)
...
def generate_quote(master_df: pd.DataFrame, req: QuoteRequest) -> Dict[str, Any]:
    ...
    for option_index, (_, row) in enumerate(df_top.iterrows(), start=1):
        ...
        option = {
            ...
            "pol": str(row["POL"]),
            "pod": str(row["POD"]),  # POD thực tế từ Master Pricing (Via Port)
            "place_of_delivery": str(row["PlaceOfDelivery"]),
            "via_port": str(row.get("POD", "")),  # giữ nguyên logic cũ, dùng POD làm via
            ...
            "transit_time": str(row.get("TransitTime", "")),
    ...
    # Lấy Via Port từ option recommended (option 1) để đưa vào summary
    via_port_for_summary = options_out[0].get("via_port") if options_out else None
    ...
    summary = {
        ...
        "pol": shipment.pol,
        "pod": shipment.pod,  # POD filter (nếu user chọn cụ thể)
        "place_of_delivery": shipment.place_of_delivery,
        "via_port": via_port_for_summary,  # Via Port thực tế từ dòng giá recommend
    ...
    return {
        ...
        "summary": summary,
        "options": options_out,
        "containers": [item.type for item in req.containers],
        "debug": debug_info,
    }
...
def pretty_print_quote(result: Dict[str, Any]) -> None:
    ...
    print(f"VIA PORT  : {s.get('via_port')}")
    print("-" * 70)
        ...
        [
            {
                ...
                "Route": summary["route"],
                "POL": summary["pol"],
                "POD_Filter": summary["pod"],           # POD user filter (nếu có)
                "ViaPort": summary.get("via_port"),     # Via Port thực tế từ dòng giá recommend
