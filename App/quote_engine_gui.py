...
def main():
    ...
    # ======= GLOBAL CSS – NEUMORPHISM CAPSULE SIDEBAR (LAYOUT A) =======
    st.markdown(
        """
    <style>
    :root {
        --pd-sidebar-bg: #F9FAFB;   /* nền sidebar */
        --pd-tab-bg: #FFFFFF;       /* nền tab thường */
        --pd-tab-active-bg: #E0F2FE;/* nền tab active */
        --pd-text: #374151;         /* chữ xám đậm */
        --pd-title: #1F2937;
        --pd-icon-normal: #60A5FA;  /* icon thường */
        --pd-icon-active: #3B82F6;  /* icon active */
    }
    ...
    /* COMMON STYLE CHO 3 BLOCK MENU */
    .pd-sidebar-menu-pricing div[role='radiogroup'],
    .pd-sidebar-menu-shipment div[role='radiogroup'],
    .pd-sidebar-menu-customers div[role='radiogroup'] {
        display: flex;
        flex-direction: column;
        gap: 0.5rem;
    }

    /* Ẩn chấm radio mặc định */
    .pd-sidebar-menu-pricing div[role='radiogroup'] input[type="radio"],
    .pd-sidebar-menu-shipment div[role='radiogroup'] input[type="radio"],
    .pd-sidebar-menu-customers div[role='radiogroup'] input[type="radio"] {
        opacity: 0;
        position: absolute;
    }

    /* Label chung (bao ngoài) */
    .pd-sidebar-menu-pricing div[role='radiogroup'] > label,
    .pd-sidebar-menu-shipment div[role='radiogroup'] > label,
    .pd-sidebar-menu-customers div[role='radiogroup'] > label {
        cursor: pointer;
    }

    /* Nội dung tab (capsule) – div đứng ngay sau input */
    .pd-sidebar-menu-pricing div[role='radiogroup'] > label > div:nth-child(2),
    .pd-sidebar-menu-shipment div[role='radiogroup'] > label > div:nth-child(2),
    .pd-sidebar-menu-customers div[role='radiogroup'] > label > div:nth-child(2) {
    ...
    /* Emoji icon + text nằm trong span */
    .pd-sidebar-menu-pricing div[role='radiogroup'] > label > div:nth-child(2) span,
    .pd-sidebar-menu-shipment div[role='radiogroup'] > label > div:nth-child(2) span,
    .pd-sidebar-menu-customers div[role='radiogroup'] > label > div:nth-child(2) span {
        font-size: 0.92rem;
    }

    /* Hover: tab nổi nhẹ hơn */
    .pd-sidebar-menu-pricing div[role='radiogroup'] > label:hover > div:nth-child(2),
    .pd-sidebar-menu-shipment div[role='radiogroup'] > label:hover > div:nth-child(2),
    .pd-sidebar-menu-customers div[role='radiogroup'] > label:hover > div:nth-child(2) {
        box-shadow: 0 10px 20px rgba(15,23,42,0.12);
        transform: translateY(-1px);
    }

    /* ACTIVE: input checked -> capsule đổi màu pastel */
    .pd-sidebar-menu-pricing div[role='radiogroup'] input[type="radio"]:checked + div,
    .pd-sidebar-menu-shipment div[role='radiogroup'] input[type="radio"]:checked + div,
    .pd-sidebar-menu-customers div[role='radiogroup'] input[type="radio"]:checked + div {
        background: var(--pd-tab-active-bg);
        border-color: #BFDBFE;
        box-shadow: 0 12px 24px rgba(37,99,235,0.22);
    }

    /* ACTIVE: chữ đậm hơn */
    .pd-sidebar-menu-pricing div[role='radiogroup'] input[type="radio"]:checked + div span,
    .pd-sidebar-menu-shipment div[role='radiogroup'] input[type="radio"]:checked + div span,
    .pd-sidebar-menu-customers div[role='radiogroup'] input[type="radio"]:checked + div span {
        color: var(--pd-title);
        font-weight: 600;
    }

    /* Ẩn option placeholder đầu tiên trong mỗi nhóm */
    .pd-sidebar-menu-pricing div[role='radiogroup'] > label:nth-child(1),
    .pd-sidebar-menu-shipment div[role='radiogroup'] > label:nth-child(1),
    .pd-sidebar-menu-customers div[role='radiogroup'] > label:nth-child(1) {
        display: none;
    }

    /* Card style – phần nội dung chính */
    .info-card {
        background: #ffffff;
        border-radius: 14px;
        padding: 14px 18px;
    ...
    if "current_menu" not in st.session_state:
        st.session_state["current_menu"] = "Upload & Normalize"

    # thêm state để track lựa chọn lần trước của 3 block
    if "pricing_choice_prev" not in st.session_state:
        st.session_state["pricing_choice_prev"] = "(none)"
    if "shipment_choice_prev" not in st.session_state:
        st.session_state["shipment_choice_prev"] = "(none)"
    if "customer_choice_prev" not in st.session_state:
        st.session_state["customer_choice_prev"] = "(none)"

    RAW_DIR.mkdir(parents=True, exist_ok=True)
    DATA_DIR.mkdir(parents=True, exist_ok=True)

    # ============================================================
    # MENU OPTIONS (KEY CHÍNH CỦA APP)
    # ============================================================
    menu_options = (
        "Upload & Normalize",   # tab normalize / upload RAW
        "Quote",
        "Schedules",
        "Follow Shipment",
        "Dashboard",
    ...
    if current_menu not in menu_options:
        current_menu = "Upload & Normalize"

    # ============================================================
    # SIDEBAR – NEUMORPHIC CAPSULE (PRICING / SHIPMENT / CUSTOMERS)
    # ============================================================
    with st.sidebar:
        st.markdown(
            "<div class='pd-sidebar-app-title'>Pudong</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "<div class='pd-sidebar-subtitle'>Logistics Pricing Suite</div>",
            unsafe_allow_html=True,
        )

        # ===== PRICING BLOCK =====
        st.markdown(
            "<div class='pd-sidebar-section-title'>PRICING</div>",
            unsafe_allow_html=True,
        )

        pricing_labels = [
            "(none)",  # placeholder để ẩn bằng CSS
            "☁️ Upload & Normalize",
            "💲 Quote",
            "📅 Schedules",
        ]
        pricing_map = {
            "☁️ Upload & Normalize": "Upload & Normalize",
            "💲 Quote": "Quote",
            "📅 Schedules": "Schedules",
        }

        # Xác định tab hiện tại nằm ở block nào
        if current_menu in pricing_map.values():
            for lbl, key in pricing_map.items():
                if key == current_menu:
                    current_pricing_label = lbl
                    break
            pricing_index = pricing_labels.index(current_pricing_label)
        else:
            pricing_index = 0  # placeholder

        st.markdown("<div class='pd-sidebar-menu-pricing'>", unsafe_allow_html=True)
        ...
        # ===== SHIPMENT BLOCK =====
        st.markdown(
            "<div class='pd-sidebar-section-title'>SHIPMENT</div>",
            unsafe_allow_html=True,
        )
        ...
        shipment_map = {
            "📦 Follow Shipment": "Follow Shipment",
            "🧭 Dashboard": "Dashboard",
        }

        if current_menu in shipment_map.values():
            for lbl, key in shipment_map.items():
                if key == current_menu:
                    current_ship_label = lbl
                    break
            shipment_index = shipment_labels.index(current_ship_label)
        else:
            shipment_index = 0

        st.markdown("<div class='pd-sidebar-menu-shipment'>", unsafe_allow_html=True)
        shipment_choice = st.radio(
            "",
            options=shipment_labels,
        ...
        # ===== CUSTOMERS BLOCK =====
        st.markdown(
            "<div class='pd-sidebar-section-title'>CUSTOMERS</div>",
            unsafe_allow_html=True,
        )
        ...
        if current_menu in customer_map.values():
            for lbl, key in customer_map.items():
                if key == current_menu:
                    current_cus_label = lbl
                    break
            customer_index = customer_labels.index(current_cus_label)
        else:
            customer_index = 0

        st.markdown("<div class='pd-sidebar-menu-customers'>", unsafe_allow_html=True)
    ...
    # ============================================================
    # QUYẾT ĐỊNH MENU THEO SIDEBAR (DỰA VÀO BLOCK VỪA THAY ĐỔI)
    # ============================================================
    prev_pricing = st.session_state["pricing_choice_prev"]
    prev_shipment = st.session_state["shipment_choice_prev"]
    prev_customer = st.session_state["customer_choice_prev"]

    changed_pricing = pricing_choice != prev_pricing
    changed_shipment = shipment_choice != prev_shipment
    changed_customer = customer_choice != prev_customer

    new_menu = current_menu

    if changed_pricing and pricing_choice in pricing_map:
        new_menu = pricing_map[pricing_choice]
    elif changed_shipment and shipment_choice in shipment_map:
        new_menu = shipment_map[shipment_choice]
    elif changed_customer and customer_choice in customer_map:
        new_menu = customer_map[customer_choice]
    else:
        # không có thay đổi hoặc chỉ chọn (none) -> giữ nguyên current_menu
        pass

    # cập nhật prev_* cho lần sau
    st.session_state["pricing_choice_prev"] = pricing_choice
    st.session_state["shipment_choice_prev"] = shipment_choice
    st.session_state["customer_choice_prev"] = customer_choice
    ...
    top_index = menu_options.index(menu)
    with col_tm2:
    ...
    # Ưu tiên nút nhanh, sau đó tới dropdown top
    if go_quote:
        menu = "Quote"
    elif go_dashboard:
        menu = "Dashboard"
    ...
    # ============================================================
    # 1) TAB: UPLOAD & NORMALIZE (Upload file giá RAW, normalize)
    # ============================================================
    if menu == "Upload & Normalize":
        st.markdown(
    ...
    try:
        ...
        st.info(
            "Anh hãy sang tab **'Upload & Normalize'** để upload RAW và bấm Normalize."
        )
        return
        ...
        if generate_btn:
            ...
            for dest in selected_places:
                ...
                with col_s3:
                    ...
                    st.markmarkdown(f"**Incoterm:** {summary.get('incoterm')}")
                    st.markdown(
