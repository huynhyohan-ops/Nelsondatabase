...
from normalize_pricing_work import normalize_all_from_streamlit  # ✅ import đúng tên file
...
def main():
    ...
    # === INIT STATE CHO VERSION PRICING & MARKUP MAP ===
    ...
    # Đảm bảo thư mục RAW & DATA tồn tại
    RAW_DIR.mkdir(parents=True, exist_ok=True)
    DATA_DIR.mkdir(parents=True, exist_ok=True)

    # ================== SIDEBAR: UPLOAD & NORMALIZE RAW ==================
    st.sidebar.header("⚙️ Cập nhật & Normalize bảng giá")

    uploaded_raw = st.sidebar.file_uploader(
        "Upload file giá RAW (.xlsx)",
        type=["xlsx"],
        accept_multiple_files=True,
        help="Upload bảng giá gốc (RAW). Hệ thống sẽ tự Normalize thành Master_FullPricing.xlsx."
    )

    if st.sidebar.button("🧹 Normalize RAW → Master", use_container_width=True):
        if not uploaded_raw:
            st.sidebar.warning("Anh cần upload ít nhất 1 file RAW trước.")
        else:
            # 1) Lưu các file RAW vào thư mục RAW
            for f in uploaded_raw:
                out_path = RAW_DIR / f.name
                with open(out_path, "wb") as g:
                    g.write(f.getbuffer())

            # 2) Gọi pipeline normalize ngay trong môi trường Streamlit
            try:
                with st.spinner("Đang Normalize bảng giá RAW → Master_FullPricing.xlsx ..."):
                    normalize_all_from_streamlit(
                        raw_dir_override=RAW_DIR,
                        data_dir_override=DATA_DIR,
                    )

                # 3) Tăng version để invalid cache & rerun app
                st.session_state["pricing_version"] += 1
                st.sidebar.success("✅ Đã Normalize & cập nhật Master Pricing thành công.")
                st.experimental_rerun()

            except Exception as e:
                st.sidebar.error(f"Lỗi khi Normalize: {e}")

    st.sidebar.markdown("---")
    st.sidebar.caption(
        f"📄 Master hiện tại: `{MASTER_FILE.name}` – version: {st.session_state['pricing_version']}"
    )
    ...
    try:
        ...
        st.info(
            "Anh hãy upload file RAW ở sidebar và bấm nút **'Normalize RAW → Master'** để tạo Master_FullPricing.xlsx trước."
        )
        st.stop()
