...
from common.models import (
    ...
    QuoteRequest,
    load_master,
    filter_by_validity,  # ✅ dùng đúng filter_by_validity từ models
)
from common.cost_engine import generate_quote  # ✅ chỉ import generate_quote
...
# ================================================================
# PREVIEW INTERNAL COST – LOGIC COPY TỪ ENGINE CŨ (ĐÃ CHỈNH)
# ================================================================
def _preview_internal_cost(
    ...
    """
    Logic dựa theo preview_cost_by_carrier trong Quote_Engine_V1,
    nhưng trả ra DataFrame đúng format anh yêu cầu:

    | # | Carrier | POD | Place of Delivery | RoutingNote | EffectiveDate |
      ExpirationDate | CommodityType | RateType | (các cột cont đã chọn)
    """
...
def render_quote_page():
    ...
    st.markdown(
        "Nhập thông tin lô hàng, chọn loại container và hệ thống sẽ đề xuất bảng giá phù hợp nhất."
    )
    ...
    if preview_btn or generate_btn:
        ...
        containers: List[ContainerPlanItem] = []
        selected_cont_types: List[str] = []   # ✅ dùng List[str] cho an toàn
        ...
        shipment = ShipmentInfo(
            ...
            cargo_ready_date=str(cargo_ready_date),
            incoterm=incoterm,
        ...
        with st.spinner("⏳ Đang tính toán..."):
            # Luôn gọi generate_quote để có options + recommended
            ...
            if preview_btn:
                # chạy preview internal cost (base cost, không markup)
                ...
                if rec_opt is not None and not df_preview.empty:
                    ...
                    if mask.any():
                        ...
                        # Đánh lại cột # theo thứ tự mới
                        df_preview["#"] = range(1, len(df_preview) + 1)

                # Lưu state cho preview
    ...
    if mode == "full":
        ...
        # Nếu anh vẫn dùng hàm generate_quote_pdf thì giữ nguyên:
