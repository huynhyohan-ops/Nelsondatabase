...
def _tt_compact(min_tt, max_tt) -> str:
    ...
    VERSION_SHEET_PATTERN = re.compile(
        r"^(?P<day>\d{1,2})(?P<month>[A-Z]{3})NO(?P<num>\d+)$",
        re.IGNORECASE,
    )

    def format_version_from_sheet_name(sheet_name: str) -> str:
        """
        '12DECNO1' -> '12DEC - NO.1'
        Không match pattern thì trả về sheet_name.upper()
        """
        value = sheet_name.strip().upper()
        m = VERSION_SHEET_PATTERN.match(value)
        if not m:
            return value

        day = str(int(m.group("day")))  # '01' -> '1'
        month = m.group("month").upper()  # 'DEC'
        num = str(int(m.group("num")))  # '01' -> '1'

        return f"{day}{month} - NO.{num}"

    VERSION_PATTERN = re.compile(r"^(?P<date>\d{1,2}[A-Z]{3})NO(?P<num>\d+)$", re.IGNORECASE)

    def _format_version_human(raw: str | None) -> str:
        """
        Chuyển '12DECNO1' -> '12DEC - NO.1'.
        Nếu không match pattern thì trả về nguyên chuỗi (upper).
        """
        if not raw:
            return "N/A"
        ...
        date_token = match.group("date")  # ví dụ '12DEC'
        month = date_token[-3:]  # 'DEC'
        day_digits = date_token[:-3]  # '12'
        day = str(int(day_digits))  # bỏ 0 thừa nếu có -> '12'

        num_token = match.group("num")  # '1' hoặc '01'
        num = str(int(num_token))  # chuẩn hóa '1'
...
# ========================== VERSION KPI ==========================
from pathlib import Path  # đã import ở đầu file rồi, không cần thêm nếu có sẵn


from pathlib import Path  # đã có sẵn ở đầu file thì không cần thêm

def get_latest_version_from_master(master_file: str | Path = MASTER_FILE) -> str:
    """
    Đọc version mới nhất từ file Master_FullPricing.xlsx dựa theo TÊN SHEET.

    - Dùng đúng MASTER_FILE mà hệ thống đang load (Data/Master_FullPricing.xlsx).
    - Lọc các sheet có tên dạng: DDMMMNOx, ví dụ '12DECNO1'.
    - Lấy sheet cuối cùng làm bản mới nhất.
    - Trả về dạng dễ đọc: '12DEC - NO.1'.
    """
    ...
    # Lọc các sheet tên đúng pattern version
    version_sheets = [
        s for s in sheet_names
        if re.match(r"^\d{1,2}[A-Z]{3}NO\d+$", s.strip().upper())
    ]
    if not version_sheets:
        return "N/A"

    # Theo quy ước: sheet version mới nhất nằm cuối
    latest_sheet_name = version_sheets[-1]
    value = latest_sheet_name.strip().upper()

    m = re.match(r"^(\d{1,2})([A-Z]{3})NO(\d+)$", value)
    if not m:
        # Nếu tên sheet khác pattern thì trả thẳng tên sheet
        return value

    day, month, num = m.groups()
    day_fmt = str(int(day))       # '01' -> '1'
    num_fmt = str(int(num))       # '01' -> '1'

    return f"{day_fmt}{month} - NO.{num_fmt}"
