...
def _preview_internal_cost(
    ...
    df_valid["ExpirationDate_dt"] = pd.to_datetime(df_valid["ExpirationDate"], errors="coerce")

    # ===============================
    # 🔁 CHỌN EXPIRATION XA NHẤT, GIÁ THẤP NHẤT
    # ===============================
    df_valid_sorted = df_valid.sort_values(
        by=["Carrier_upper", "ExpirationDate_dt", "TotalBase"],
        ascending=[True, False, True],
    )

    idx_best = df_valid_sorted.groupby("Carrier_upper").head(1).index
